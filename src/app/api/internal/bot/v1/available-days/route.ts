...
export const POST = withBotInternalApi(async (request: Request) => {
  ...
  const parsed = parseBotAvailableDaysBody(bodyResult.value);
  if (!parsed.ok) {
  ...
  try {
    ...
    const result = await evaluateBotAvailableDays(
      parsed.value,
      studioToday,
      now,
    );
    return mapBotAvailabilityDomainResult(
      "bot-internal-available-days",
      result,
    );
  } catch (error) {
    return botAvailabilityInternalErrorResponse(
      "bot-internal-available-days",
      error,
    );
  }
