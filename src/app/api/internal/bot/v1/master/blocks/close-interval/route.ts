...
import { masterCloseInterval } from "@/services/MasterCommandService";
...
export const POST = withBotInternalApi(
  async (request: Request) => {
    ...
    const parsed = parseMasterCloseIntervalBody(bodyResult.value);
    if (!parsed.ok) {
      return masterCommandErrorResponse("VALIDATION_ERROR", 400);
    }

    try {
      const result = await masterCloseInterval(parsed.value);
      if (!result.ok) {
        return masterCommandErrorResponse(result.code, result.httpStatus);
      }
      return masterCommandSuccessResponse(result.body);
    } catch (error) {
      safeLogError("bot-internal-master-close-interval", error);
