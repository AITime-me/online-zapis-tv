...
import { getMasterSchedule } from "@/services/MasterCommandService";
...
export const POST = withBotInternalApi(
  async (request: Request) => {
    ...
    const parsed = parseMasterScheduleReadBody(bodyResult.value);
    if (!parsed.ok) {
      return masterCommandErrorResponse("VALIDATION_ERROR", 400);
    }

    try {
      const result = await getMasterSchedule(parsed.value);
      if (!result.ok) {
        return masterCommandErrorResponse(result.code, result.httpStatus);
      }
      return masterCommandSuccessResponse(result.body);
    } catch (error) {
      safeLogError("bot-internal-master-schedule", error);
      return masterCommandErrorResponse("INTERNAL_ERROR", 500);
    }
  },
  { rateLimitPolicy: "botInternal" },
);
