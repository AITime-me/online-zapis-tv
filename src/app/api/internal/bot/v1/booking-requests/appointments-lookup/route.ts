...
import { lookupBotBookingRequestAppointments } from "@/services/BotBookingRequestService";
...
export const POST = withBotInternalApi(async (request: Request) => {
  ...
  const parsed = parseBotBookingRequestAppointmentsLookupBody(bodyResult.value);
  if (!parsed.ok) {
    return errorResponse("VALIDATION_ERROR", 400);
  }

  try {
    const result = await lookupBotBookingRequestAppointments(parsed.value);
    if (!result.ok) {
    ...
    safeLogError("bot-internal-booking-requests-appointments-lookup", error);
