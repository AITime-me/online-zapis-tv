...
import { feedBotBookingRequests } from "@/services/BotBookingRequestService";
...
export const POST = withBotInternalApi(async (request: Request) => {
  ...
  const parsed = parseBotBookingRequestFeedBody(bodyResult.value);
  if (!parsed.ok) {
    return errorResponse("VALIDATION_ERROR", 400);
  }

  try {
    const result = await feedBotBookingRequests(parsed.value);
    if (!result.ok) {
    ...
    safeLogError("bot-internal-booking-requests-feed", error);
