...
import { getBotBookingRequestAvailability } from "@/services/BotBookingRequestService";
...
export const POST = withBotInternalApi(async (request: Request) => {
  ...
  const parsed = parseBotBookingRequestAvailabilityBody(bodyResult.value);
  if (!parsed.ok) {
    return errorResponse("VALIDATION_ERROR", 400);
  }

  try {
    const result = await getBotBookingRequestAvailability(parsed.value);
    if (!result.ok) {
    ...
    safeLogError("bot-internal-booking-requests-availability", error);
