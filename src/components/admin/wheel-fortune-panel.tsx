...
export function WheelFortunePanel({
  ...
  const saveGift = async () => {
    ...
    try {
      ...
      if (!wheelResponse.ok || !wheelPayload.ok) {
        throw new Error(wheelPayload.error ?? "Не удалось обновить данные");
      }
      setGifts(wheelPayload.gifts);
      setWheelConfig(wheelPayload.wheelConfig);
      setTitle(wheelPayload.title);
      setSlug(wheelPayload.slug);
      setDescription(wheelPayload.description ?? "");
      setCatalogStatus(parseCatalogStatus(wheelPayload.status));
      if (typeof wheelPayload.showOnHomepage === "boolean") {
        setShowOnHomepage(wheelPayload.showOnHomepage);
      }
      setEditingGiftId(null);
      const nextStatus = parseCatalogStatus(wheelPayload.status);
      const sectorOk = Boolean(wheelPayload.wheelConfig?.sectorConfigOk);
      markSaved(
        generation,
        nextStatus === "active" && !sectorOk
          ? "Сохранено. Сумма активных секторов некорректна — исправьте конфигурацию."
          : null,
      );
    } catch (error) {
      if (generation === requestGenerationRef.current) {
