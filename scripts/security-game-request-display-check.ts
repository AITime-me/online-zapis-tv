/**
 * Compact game booking display for schedule + gift-edit safety guards.
 */
...
function assertWheelCompactDisplay(): void {
  const play = basePlay({
    ...
    giftSnapshot: {
      ...buildGiftSnapshot(
        {
          id: GIFT_ID,
          name: "Биоревитализант губ",
          shortDescription: "Улучшение услуги",
          image: null,
          priority: "standard",
          cardStyle: "default",
          activationMode: "SINGLE_PAID_SERVICE",
      ...
      confirmedZone: "lips",
      finalPrize: { giftId: GIFT_ID, name: "Биоревитализант губ", systemKey: "lips_bio" },
    },
  });
  ...
  assert.equal(display!.procedure, null);
  assert.equal(display!.zone, "Перманент губ");
  assert.equal(display!.giftName, "Биоревитализант губ");
  assert.equal(display!.clientMessage, null);

  const compact = formatGameBookingRequestCompactComment(display!);
  assert.equal(
    compact,
    ["Зона: Перманент губ", "Подарок: Биоревитализант губ"].join("\n"),
  );
  assert.doesNotMatch(compact, /Клиент прошёл игру/);
  assert.doesNotMatch(compact, /Условие получения/);
  assert.doesNotMatch(compact, /Срок действия/);
...
function assertProcedureAndClientMessage(): void {
  ...
  assert.match(compact, /^Подарок: Скидка 15%$/m);
