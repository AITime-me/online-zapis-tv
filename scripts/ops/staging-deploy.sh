...
usage() {
  cat <<'EOF'
Usage: scripts/ops/staging-deploy.sh [--dry-run] [--yes] [--redeploy-current] [--help]
...
Options:
  ...
  --redeploy-current   Rebuild and redeploy the current commit when HEAD already
                       matches origin/main (e.g. after manual git pull or a
                       previously interrupted deploy). Does not change Git state.
  --help               Show this help
...
parse_args() {
  ...
  if [[ "$DEPLOY_HELP" -eq 1 ]]; then
    if [[ "$OPS_DRY_RUN" -eq 1 || "$DEPLOY_YES" -eq 1 || "$DEPLOY_REDEPLOY_CURRENT" -eq 1 ]]; then
...
collect_app_image_state() {
  ...
  CURRENT_CONTAINER_IMAGE_ID="$(ops_get_container_image_id "$STAGING_APP_CONTAINER" | ops_normalize_image_id)"
  if [[ -z "$CURRENT_CONTAINER_IMAGE_ID" ]]; then
    ops_die "cannot determine current app container image id"
  fi

  CURRENT_CONTAINER_IMAGE_REF="$(ops_get_container_image_reference "$STAGING_APP_CONTAINER" || true)"
...
print_plan() {
  ...
  if [[ "$DEPLOY_REDEPLOY_CURRENT" -eq 1 ]]; then
    ...
    ops_info "  compose app image ref:       ${STAGING_APP_IMAGE_REF}"
    ...
    if [[ -n "$COMPOSE_APP_IMAGE_ID" && "$COMPOSE_APP_IMAGE_ID" != "$CURRENT_CONTAINER_IMAGE_ID" ]]; then
      ops_info "  note: legacy or stale image reference is expected until redeploy completes"
    fi
    ops_info "  action: current commit will be rebuilt and redeployed"
    ops_info "  backup: will be created before migrations"
    ops_info "  migrations: status checked; deploy only if pending"
    ops_info "  postgres: will not be restarted"
    ops_info "  rollback: previous container image id will be tagged for app rollback"
  fi

  ops_info "Steps:"
  if [[ "$DEPLOY_REDEPLOY_CURRENT" -eq 1 ]]; then
    ops_info "  1. Validate environment, compose config, and git state (no git merge)"
    ops_info "  2. Create and verify PostgreSQL backup"
    ops_info "  3. Tag current app container image id for rollback"
    ...
    ops_info "  7. Restart app container only"
    ops_info "  8. Wait for Docker health and HTTP /api/health"
    ops_info "  9. Update deploy state manifest after each critical stage"
  else
