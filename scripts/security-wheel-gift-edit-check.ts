...
function assertGameAdminGiftGuards(): void {
  ...
  const deleteFn = service.match(
    /export async function deleteGameGift\([\s\S]*?\n\}/,
  );
  assert.ok(deleteFn, "deleteGameGift must exist");
  assert.match(deleteFn[0]!, /gamePlay\.count/);
  assert.match(deleteFn[0]!, /selectedGiftId:\s*id/);
  assert.match(deleteFn[0]!, /status:\s*["']ACTIVE["']/);
  assert.match(deleteFn[0]!, /serverAssignmentReferencesGiftId/);
  assert.match(deleteFn[0]!, /Отключите его вместо удаления/);
}
