...
## Runtime header smoke (после build)

Source-level `test:security:manage-link-hardening` недостаточен для Cache-Control.
...
Либо против уже запущенного staging/local server:

```bash
MANAGE_SMOKE_BASE_URL=http://127.0.0.1:3000 npm run test:security:manage-link-headers-smoke
```

Проверяет итоговые HTTP headers manage page/API (no-store, no-referrer, не prerender HIT).
