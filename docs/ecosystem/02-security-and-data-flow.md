...
2. Bot Core обрабатывает тексты диалогов в своём контуре (собственный PostgreSQL;
   foundation = PG-only; Redis не требуется, пока нет отдельной runtime-необходимости).
